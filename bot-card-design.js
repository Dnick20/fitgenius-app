...
// Card Design Bot - Phase 4
// Creates expandable linear card components

import fs from 'fs';

class CardDesignBot {
  constructor() {
    this.botName = 'Card Design Bot';
  ...
  async execute() {
    ...
    try {
      await this.createExpandableCards();
      await this.createCardStyles();
  ...
  async createExpandableCards() {
    console.log('  🎴 Creating expandable card components...');
    
    const expandableCard = `import React, { useState } from 'react';
...
export default { ExpandableCard, MealCard, WorkoutCard };`;
    ...
    fs.writeFileSync('./src/components/ExpandableCards.jsx', expandableCard);
    console.log('  ✅ Expandable card components created');
  }

  async createCardStyles() {
    console.log('  🎨 Creating card-specific styles...');
    
    const cardStyles = `/* Expandable Card Styles */
.linear-card {
...
@media (max-width: 768px) {
  ...
  .linear-card h4 {
    ...
    fs.writeFileSync('./src/styles/cards.css', cardStyles);
    console.log('  ✅ Card styles created');
  }
}

if (process.argv[1] === new URL(import.meta.url).pathname) {
  const bot = new CardDesignBot();
  bot.execute().then(() => process.exit(0)).catch(() => process.exit(1));
}

export default CardDesignBot;